...
class Currency:
    ...
    def get_price(self, quote, base, amount):
        amount = int(amount)
        currency_dict = self.get_currency()  # Получаем словарь валют

        available_currencies = {"Рубль": "RUB"}            # Инвертируем значение в ключ,
        # так как словарь валют имеет вид "USD": "Доллар США"
        ...
        if quote == "RUB":
            ...
            return result
        if base == "RUB":
            result = str(round(((1 / currency_dict[quote]["Value"]) * amount), 2))
            print(result)
            return result
        ...
        result = round((price_quote / price_base) * amount, 2)
        return str(result)
