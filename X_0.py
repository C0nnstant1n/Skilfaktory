...
def write_matrix(x_y, ch):                  # Запись хода в матрицу
    if game_desk[x_y[0]][x_y[1]] != "-":    # Проверям на повтор хода
...
def extract_column(matrix):     # Извлекаем столбец из матрицы для проверки
    for i in range(3):
        ...
        for j in range(3):
            a += matrix[j][i]
        if a.count("x") == 3 or "ooo" == a:  # a.count писать длиннее(да и процу считать дольше),
                                             # поэтому просто сравниваем строку))
            return True
    return False
