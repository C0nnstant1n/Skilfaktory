...
player = User(w,
              h)  # Создаём игрока и заодно список кораблей, в задании ничего не сказано о том что игрок должен расставлять корабли вручную
comp = User(w, h)
...
while True:
    try:  # С помощью исключений определяем победу или поражение
